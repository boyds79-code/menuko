...
EAS 프로젝트는 이미 연결돼 있습니다 (`app.json`의 `extra.eas.projectId`,
`@chris_kim/menuko-staff`). 실기기(시뮬레이터/에뮬레이터는 푸시 토큰을 못 받는 경우가
많음)에서 로그인하면 `device_push_tokens` 테이블에 토큰이 저장되고 바로 동작합니다.
...
- [ ] 앱을 백그라운드로 보내고 다른 기기에서 새 주문 생성 → 푸시 알림이 오는지
      (시뮬레이터에서는 안 될 수 있음, 실기기 필요)
